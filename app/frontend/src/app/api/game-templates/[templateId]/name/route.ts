...
import { RenameGameTemplateRequestSchema } from '@shared/types/api/schemas';
...
export async function PATCH(
    ...
    try {
        ...
        const validationResult = RenameGameTemplateRequestSchema.safeParse(body);
        ...
        console.log('Renaming game template:', { templateId, name: body.name });

        // Forward request to backend API
        const backendResponse = await fetch(
            `${BACKEND_API_BASE_URL}/game-templates/${templateId}/name`,
            {
                method: 'PATCH',
                headers: {
        ...
        console.error('Error in game template rename API route:', error);
        return NextResponse.json(
