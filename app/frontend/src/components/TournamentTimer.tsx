...
function formatTimer(val: number | null) {
    if (val === null || val === undefined || val < 0) return '0'; // Show "0" instead of "-" when timer reaches zero
    const rounded = Math.max(0, Math.floor(val)); // Use floor for countdown experience
    if (rounded >= 60) {
