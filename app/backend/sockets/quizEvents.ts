    ...
    if (!quizState[quizId]) {
        ...
        try {
            ...
            if (quiz && quiz.questions_ids) {
                const orderedQuestions: any[] = []; // Using any[] temporarily until Question type is properly defined
...
function registerQuizEvents(io: Server, socket: Socket, prisma: PrismaClient): void {
    ...
    socket.on("quiz_set_question", (payload) => {
        ...
        if (!quizState[quizId].askedQuestions) {
            quizState[quizId].askedQuestions = new Set<string>();
        }

        // Add the current question UID to the askedQuestions set
        const currentQuestion = quizState[quizId].questions.find((q: any) => q.uid === quizState[quizId].currentQuestionUid);
