...
import { QuestionState } from '../types/tournamentTypes';
...
// Define LeaderboardEntry locally since it doesn't match what computeLeaderboard returns
interface LeaderboardEntry {
    id: string;
    pseudo: string; // Using pseudo instead of name to match tournamentTypes
    avatar?: string;
    score: number;
    [key: string]: any;
}
...
function handleCloseQuestion(
    ...
    try {
        ...
        if (tState && tState.participants) {
            ...
            const askedQuestionsRecord: Record<string, QuestionState> = {};
            tState.askedQuestions.forEach(uid => {
                ...
                if (question) {
                    ...
                    let correctAnswers: number | number[];
