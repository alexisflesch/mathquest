// filepath: /home/aflesch/mathquest/app/sockets/tournamentEventHandlers/resumeHandler.js
/**
...
try {
    ...
    try {
        ...
        tsModule = require('./resumeHandler');
        ...
        if (typeof tsModule === 'function') {
            ...
            console.warn('Could not find resumeHandler function directly, trying delayed import');
            // Give time for circular dependency resolution
            setTimeout(() => {
                try {
                    const reloadedModule = require('./resumeHandler');
        ...
        tsModule = function (io, socket, payload) {
            console.error('Using fallback resumeHandler - TypeScript module could not be loaded');
            socket.emit('tournament_action_response', {
                status: 'error',
                message: 'Internal server error: Tournament resume handler not available'
            });
        };
    }
} catch (error) {
    console.error('Error setting up resumeHandler:', error);

    // Final fallback
    tsModule = function (io, socket, payload) {
        console.error('Using emergency fallback resumeHandler');
        socket.emit('tournament_action_response', {
            status: 'error',
            message: 'Internal server error: Tournament resume handler not available'
        });
