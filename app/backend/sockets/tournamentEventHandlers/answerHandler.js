/**
 * answerHandler.js - JavaScript bridge to TypeScript handler
 * 
...
try {
    ...
    try {
        ...
        tsModule = require('./answerHandler');
        ...
        if (typeof tsModule === 'function') {
            ...
            console.warn('Could not find answerHandler function directly, trying delayed import');
            // Give time for circular dependency resolution
            setTimeout(() => {
                try {
                    const reloadedModule = require('./answerHandler');
        ...
        tsModule = function (io, socket, payload) {
            console.error('Using fallback answerHandler - TypeScript module could not be loaded');
            socket.emit('tournament_answer_response', {
                status: 'error',
                message: 'Internal server error: Tournament answer handler not available'
            });
        };
    }
} catch (error) {
    console.error('Error setting up answerHandler:', error);

    // Final fallback
    tsModule = function (io, socket, payload) {
        console.error('Using emergency fallback answerHandler');
        socket.emit('tournament_answer_response', {
            status: 'error',
            message: 'Internal server error: Tournament answer handler not available'
        });
