// filepath: /home/aflesch/mathquest/app/sockets/tournamentEventHandlers/startHandler.js
/**
 * startHandler.js - JavaScript bridge to TypeScript handler
 * 
...
try {
    ...
    try {
        ...
        if (typeof tsModule === 'function') {
            ...
            console.warn('Could not find startHandler function directly, trying delayed import');
            // Give time for circular dependency resolution
            setTimeout(() => {
                try {
                    const reloadedModule = require('./startHandler');
        ...
        tsModule = function (io, socket, payload) {
            console.error('Using fallback startHandler - TypeScript module could not be loaded');
            socket.emit('tournament_action_response', {
                status: 'error',
                message: 'Internal server error: Tournament start handler not available'
            });
        };
    }
} catch (error) {
    console.error('Error setting up startHandler:', error);

    // Final fallback
    tsModule = function (io, socket, payload) {
        console.error('Using emergency fallback startHandler');
        socket.emit('tournament_action_response', {
            status: 'error',
            message: 'Internal server error: Tournament start handler not available'
        });
