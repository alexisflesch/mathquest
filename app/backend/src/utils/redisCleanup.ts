...
const logger = createLogger('RedisCleanup');
 ...
 */
export async function cleanupGameRedisKeys(accessCode: string, context: string = 'cleanup'): Promise<void> {
 ...
 */
export async function cleanupDeferredSessionRedisKeys(
    accessCode: string,
    userId: string,
    attemptCount: number,
    context: string = 'deferredSession'
): Promise<void> {
...
async function performCleanup(
    ...
    accessCode: string,
    context: string,
    additionalInfo?: { userId?: string; attemptCount?: number }
): Promise<void> {
    let totalCleaned = 0;
 ...
 */
export function getGameRedisKeyPatterns(accessCode: string): string[] {
    return [
        // Live game keys
        `mathquest:game:participants:${accessCode}`,
        `mathquest:game:leaderboard:${accessCode}`,
