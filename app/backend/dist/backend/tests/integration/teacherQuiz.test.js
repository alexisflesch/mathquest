...
function waitForEvent(socket, event, timeout = 5000) {
    return new Promise((resolve, reject) => {
        if (!socket)
            return reject(new Error('Socket is undefined while waiting for ' + event));
...
describe('Teacher-driven Quiz Flow', () => {
    ...
    let player1User;
    let quizTemplate;
    let gameInstance; // This will be our "quiz"
    let teacherToken;
    let player1Token;
    let quizId; // Will be gameInstance.id
    let accessCode;
    let questionUid; // UID of the first question
    beforeAll(async () => {
        const serverSetup = (0, server_1.setupServer)(); // Assuming this sets up Socket.IO server as well
        ...
        player1User = await prisma_1.prisma.user.create({
            data: {
        ...
        teacherToken = jsonwebtoken_1.default.sign({ userId: teacherUser.id, username: teacherUser.username, role: teacherUser.role }, secret, { expiresIn: '1h' });
        player1Token = jsonwebtoken_1.default.sign({ userId: player1User.id, username: player1User.username, role: player1User.role }, secret, { expiresIn: '1h' });
        ...
        await prisma_1.prisma.question.upsert({
            ...
            update: { ...questionData, answerOptions: questionData.answerOptions, correctAnswers: questionData.correctAnswers },
            create: { ...questionData, answerOptions: questionData.answerOptions, correctAnswers: questionData.correctAnswers },
        });
        ...
        const quizTemplateRes = await (0, supertest_1.default)(baseUrl)
        ...
        const gameInstanceRes = await (0, supertest_1.default)(baseUrl)
            .post('/api/v1/games')
            .set('Authorization', `Bearer ${teacherToken}`)
            .send({
            name: 'Live Quiz Session 1',
        ...
        await new Promise((resolve) => teacherSocket.on('connect', () => resolve()));
        ...
        player1Socket = (0, socket_io_client_1.io)(baseUrl, {
    ...
    afterAll(async () => {
        ...
        if (gameInstance)
            await prisma_1.prisma.gameInstance.deleteMany({ where: { id: gameInstance.id } });
        if (quizTemplate) {
            await prisma_1.prisma.questionsInGameTemplate.deleteMany({ where: { gameTemplateId: quizTemplate.id } });
            await prisma_1.prisma.gameTemplate.deleteMany({ where: { id: quizTemplate.id } });
        }
        await prisma_1.prisma.question.deleteMany({ where: { uid: questionUid } });
        // Delete profiles before users
        await prisma_1.prisma.studentProfile.deleteMany({ where: { id: { in: [player1User?.id].filter(Boolean) } } });
    ...
    beforeEach(() => {
        if (teacherSocket)
            teacherSocket.offAny();
        if (player1Socket)
            player1Socket.offAny();
    });
    ...
    it('teacher should be able to set a question, and player receives it', async () => {
        ...
        const questionPayload = await gameQuestionPromise;
        ...
        const dashboardUpdate = await dashboardQuestionChangedPromise;
    ...
    it('should allow teacher to control timer using quiz_timer_action', async () => {
        ...
        let playerPayload = await resumePlayerPromise;
        expect(playerPayload.timer.isPaused).toBe(false); // Timer should be running
        let teacherState = await resumeTeacherPromise;
