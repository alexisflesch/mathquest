...
describe('Self-Paced (Practice) Mode', () => {
    ...
    beforeAll(async () => {
        ...
        playerToken = jsonwebtoken_1.default.sign({ userId: playerId, username: 'Practice Player', role: 'STUDENT' }, secret, { expiresIn: '1h' });
        // Create a template and two questions
        const template = await prisma_1.prisma.gameTemplate.create({
            data: {
        ...
        const q1 = await prisma_1.prisma.question.create({
            data: {
                title: 'Practice Q1',
                text: 'What is 1+1?',
        ...
        const q2 = await prisma_1.prisma.question.create({
            data: {
                title: 'Practice Q2',
                text: 'What is 2+2?',
                questionType: questionTypes_1.QUESTION_TYPES.MULTIPLE_CHOICE_SINGLE_ANSWER,
    ...
    test('Player can play a self-paced game and receive feedback and score', async () => {
        ...
        await gameStateService_1.default.initializeGameState(gameInstanceId);
        ...
        socket.emit('start_game', { accessCode, userId: playerId }, (ack) => {
            console.log('start_game event acknowledged:', ack);
        });
        // 3. Should receive first question
        const firstQPromise = new Promise((resolve) => {
