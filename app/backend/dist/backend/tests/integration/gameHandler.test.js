...
let gameInstanceIdDb;
let firstQuestionUidDb;
let userIdDb;
...
describe('Game Handler', () => {
    ...
    beforeAll(async () => {
        ...
        const createdGameInstance = await prisma_1.prisma.gameInstance.create({
            data: {
        ...
        // });
        if (gameInstanceIdDb) {
            await gameStateService_1.default.initializeGameState(gameInstanceIdDb);
        }
    ...
    const createSocketClient = (query = {}) => {
        ...
        if (!query.token && query.userId) {
            query.token = (0, jwt_1.default)(query.userId, query.username || 'Test Player', 'STUDENT');
        }
    ...
    test('Multiple players can join a game and see each other', async () => {
        ...
        const participantEntries = await redis_1.redisClient.hgetall(`mathquest:game:participants:${TEST_ACCESS_CODE}`);
        ...
        const player1 = participantsData.participants.find((p) => p.userId === 'player-1');
        const player2 = participantsData.participants.find((p) => p.userId === 'player-2');
