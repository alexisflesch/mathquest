...
describe('Tournament Flow - Extended Tests', () => {
    ...
    beforeAll(async () => {
        ...
        player1 = await prisma_1.prisma.user.create({ data: { username: 'p1-extended', role: 'STUDENT', studentProfile: { create: { cookieId: 'cookie-p1-extended' } } } });
        player2 = await prisma_1.prisma.user.create({ data: { username: 'p2-extended', role: 'STUDENT', studentProfile: { create: { cookieId: 'cookie-p2-extended' } } } });
    ...
    afterAll(async () => {
        ...
        try {
            ...
            await prisma_1.prisma.user.deleteMany({ where: { username: { in: ['p1-extended', 'p2-extended'] } } });
    ...
    it('P1 creates, P2 joins, both play 2 questions with feedback checks', async () => {
        ...
        const gameTemplate = await prisma_1.prisma.gameTemplate.create({
            data: {
                ...
                questions: {
                    create: [
                        { questionUid: testQuestions_1.testQuestions[0].uid, sequence: 1 },
                        { questionUid: testQuestions_1.testQuestions[1].uid, sequence: 2 }
                    ]
                }
            }
        });
        ...
        const correctIndex1 = q1p1.question.correctAnswers.findIndex((v) => v);
        const correctAnswer1 = q1p1.question.answerOptions[correctIndex1];
        const wrongIndex1 = q1p1.question.correctAnswers.findIndex((v, idx) => !v && idx !== correctIndex1);
        ...
        expect(ca1.questionUid).toBe(testQuestions_1.testQuestions[0].uid);
        expect(ca2.questionUid).toBe(testQuestions_1.testQuestions[0].uid);
        ...
        if (q1p1.question.feedbackWaitTime && q1p1.question.feedbackWaitTime > 0) {
            ...
            expect(fb1.questionUid).toBe(testQuestions_1.testQuestions[0].uid);
            expect(fb2.questionUid).toBe(testQuestions_1.testQuestions[0].uid);
        }
        ...
        expect(q2p1.question.uid).toBe(testQuestions_1.testQuestions[1].uid);
        expect(q2p2.question.uid).toBe(testQuestions_1.testQuestions[1].uid);
        // Player 1 answers incorrectly, Player 2 answers correctly for Q2
        const correctIndex2 = q2p2.question.correctAnswers.findIndex((v) => v);
        const correctAnswer2 = q2p2.question.answerOptions[correctIndex2];
        const wrongIndex2 = q2p2.question.correctAnswers.findIndex((v, idx) => !v && idx !== correctIndex2);
        ...
        expect(caQ2_1.questionUid).toBe(testQuestions_1.testQuestions[1].uid);
        expect(caQ2_2.questionUid).toBe(testQuestions_1.testQuestions[1].uid);
        ...
        if (q2p1.question.feedbackWaitTime && q2p1.question.feedbackWaitTime > 0) {
            ...
            expect(fbQ2_1.questionUid).toBe(testQuestions_1.testQuestions[1].uid);
            expect(fbQ2_2.questionUid).toBe(testQuestions_1.testQuestions[1].uid);
        }
    });
    it('P3 joins after Q1 answers: receives Q1 with timer and correct_answers payload, cannot answer Q1', async () => {
        ...
        const player3 = await prisma_1.prisma.user.create({ data: { username: 'p3', role: 'STUDENT', studentProfile: { create: { cookieId: 'cookie-p3' } } } });
        const jwt = require('jsonwebtoken');
        const secret = process.env.JWT_SECRET || 'mathquest_default_secret';
        const signToken = (user) => jwt.sign({ userId: user.id, username: user.username, role: user.role }, secret, { expiresIn: '1h' });
        const player3Token = signToken(player3);
        // Join lobby (HTTP)
        await (0, supertest_1.default)(address).post(`/api/v1/games/${accessCode1}/join`).send({ userId: player3.id }).set('Authorization', `Bearer ${player3Token}`).expect(200);
        // Connect socket for P3
        const socket3 = (0, socket_io_client_1.io)(address, { auth: { token: player3Token }, path: '/api/socket.io', transports: ['websocket'], forceNew: true });
        await new Promise((res) => socket3.on('connect', () => res()));
        // Join tournament via socket
        ...
        const collectPayload = (event) => (data) => receivedPayloads.push({ event, data });
        ...
        await new Promise((resolve, reject) => {
            ...
            const check = () => {
                if (receivedPayloads.find(p => p.event === 'game_question') &&
        ...
        expect(qPayload.data.question.uid).toBe(testQuestions_1.testQuestions[0].uid);
        expect(caPayload.data.questionUid).toBe(testQuestions_1.testQuestions[0].uid);
        expect(fbPayload.data.questionUid).toBe(testQuestions_1.testQuestions[0].uid);
        ...
        await new Promise((resolve) => {
            socket3.once('answer_feedback', (data) => {
                answerError = data;
                resolve();
            });
            ...
            socket3.emit('tournament_answer', {
                ...
                answer: testQuestions_1.testQuestions[0].answerOptions[0], // use known answer
                timeSpent: 1
            });
