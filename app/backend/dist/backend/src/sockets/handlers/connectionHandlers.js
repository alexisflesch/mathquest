...
function registerConnectionHandlers(io) {
    io.on('connection', (socket) => {
        ...
        socket.on('disconnect', async (reason) => {
            ...
            const { disconnectHandler: gameDisconnectHandler } = await Promise.resolve().then(() => __importStar(require('./game/disconnect')));
            await gameDisconnectHandler(io, socket)();
            // Handle teacher dashboard disconnect logic
            const { disconnectHandler: teacherDisconnectHandler } = await Promise.resolve().then(() => __importStar(require('./teacherControl/disconnect')));
            await teacherDisconnectHandler(io, socket)();
            // Handle main disconnect logic (for general participant tracking)
            const { disconnectHandler: mainDisconnectHandler } = await Promise.resolve().then(() => __importStar(require('./disconnectHandler')));
