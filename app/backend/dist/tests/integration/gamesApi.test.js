...
describe('Games API Integration Tests', () => {
    ...
    mockGameInstanceService.createGameInstance.mockResolvedValue(createdGame);
    const response = await(0, supertest_1.default)(server_1.app)
        .post('/api/v1/games')
        .send(gameData)
        .expect('Content-Type', /json/)
        .expect(201);
    expect(mockGameInstanceService.createGameInstance).toHaveBeenCalledWith('teacher-123', expect.objectContaining(gameData));
    expect(response.body).toEqual({ gameInstance: createdGame });
});
it('should return 400 if required fields are missing', async () => {
    const incompleteData = {
        name: 'Missing Fields'
        // Missing gameTemplateId and playMode
    };
    const response = await (0, supertest_1.default)(server_1.app)
        .post('/api/v1/games')
        .send(incompleteData)
...
it('should return 403 if teacher does not own the game', async () => {
    ...
    const mockGame = {
        id: gameId,
                : 'different-teacher' // Different teacher
};
mockGameInstanceService.getGameInstanceById.mockResolvedValue(mockGame);
const response = await(0, supertest_1.default)(server_1.app)
    .put(`/api/v1/games/${gameId}/status`)
