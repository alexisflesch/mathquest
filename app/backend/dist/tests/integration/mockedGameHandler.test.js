...
describe('Direct handler unit tests', () => {
    ...
    test('requestParticipantsHandler emits participants list', async () => {
        ...
        await redis_1.redisClient.hset(`${PARTICIPANTS_KEY_PREFIX}${TEST_ACCESS_CODE}`, 'socket-id-1', JSON.stringify({ id: 'socket-id-1', playerId: 'player-1', username: 'Test', avatarUrl: '', joinedAt: Date.now(), score: 0, online: true }));
    ...
    test('disconnectHandler removes participant from Redis', async () => {
        const socket = createMockSocket();
        await redis_1.redisClient.hset(`${PARTICIPANTS_KEY_PREFIX}${TEST_ACCESS_CODE}`, socket.id, JSON.stringify({ id: socket.id, playerId: 'player-1', username: 'Test', avatarUrl: '', joinedAt: Date.now(), score: 0, online: true }));
...
describe('Mocked Game Handler Tests', () => {
    ...
    test('Player can request participants list', async () => {
        ...
        if (gameParticipantsCall) {
            ...
            const p1 = participantsData.participants.find((p) => p.playerId === 'player-1');
            const p2 = participantsData.participants.find((p) => p.playerId === 'player-2');
    ...
    describe('Differed Mode - Game Handler', () => {
        ...
        beforeAll(async () => {
            ...
            testTeacher = await prisma_1.prisma.teacher.upsert({
...
test('Player cannot replay a differed game after completion', async () => {
    // Mark as completed
    const participant = await prisma_1.prisma.gameParticipant.findFirst({ where: { gameInstanceId: differedGameId, playerId: 'player-1' } });
