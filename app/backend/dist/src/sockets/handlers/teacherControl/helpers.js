...
async function getGameControlState(gameId, teacherId, isTestEnvironment = false) {
    ...
    try {
        ...
        const gameInstance = await prisma_1.prisma.gameInstance.findUnique({
            where: {
                id: gameId,
                ...(isTestEnvironment ? {} : { : teacherId }) // Skip teacher ID check in test environment
            }
        });
        if (!gameInstance) {
        ...
        const questions = gameTemplate.questions.map((q) => {
            ...
            let responses = [];
            try {
                responses = typeof question.responses === 'string'
                    ? JSON.parse(question.responses)
                    : (Array.isArray(question.responses)
                        ? question.responses
                        : []);
            }
            catch (e) {
                logger.error({ questionId: question.uid, error: e }, 'Failed to parse question responses');
            }
            return {
                uid: question.uid,
                ...
                responses: responses // Include correct answers for teachers
            };
        });
