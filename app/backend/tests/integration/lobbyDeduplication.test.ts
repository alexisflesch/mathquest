...
describe('Lobby Deduplication Tests', () => {
    ...
    test('should deduplicate participants when same user joins with multiple connections', async () => {
        ...
        const socket1 = createSocketClient({
            token: generateStudentToken('user-1', 'Test User'),
            role: 'player'
        });
        ...
        // Connect all sockets
        await Promise.all([
            new Promise<void>(resolve => socket1.connect() && socket1.on('connect', () => resolve())),
            new Promise<void>(resolve => socket2.connect() && socket2.on('connect', () => resolve())),
            new Promise<void>(resolve => socket3.connect() && socket3.on('connect', () => resolve()))
        ]);
    ...
    test('should handle multiple different users correctly while deduplicating same users', async () => {
        ...
        const socket1User1 = createSocketClient({
            token: generateStudentToken('user-1', 'User One'),
            role: 'player'
        });
        ...
        const socket1User2 = createSocketClient({
            token: generateStudentToken('user-2', 'User Two'),
            role: 'player'
        });
        ...
        // Connect all sockets
        await Promise.all([
            new Promise<void>(resolve => socket1User1.connect() && socket1User1.on('connect', () => resolve())),
            new Promise<void>(resolve => socket2User1.connect() && socket2User1.on('connect', () => resolve())),
            new Promise<void>(resolve => socket1User2.connect() && socket1User2.on('connect', () => resolve()))
        ]);
        ...
        const userIds = participantsResponse.participants.map((p: any) => p.userId).sort();
        expect(userIds).toEqual(['user-1', 'user-2']);

        // Verify user-1 has the latest socket connection
        const user1Participant = participantsResponse.participants.find((p: any) => p.userId === 'user-1');
        ...
        const user2Participant = participantsResponse.participants.find((p: any) => p.userId === 'user-2');
    ...
    test('should clean up participants properly when connections disconnect', async () => {
        ...
        const socket1 = createSocketClient({
            token: generateStudentToken('user-cleanup', 'Cleanup User'),
            role: 'player'
        });
        const socket2 = createSocketClient({
            token: generateStudentToken('user-cleanup', 'Cleanup User'),
