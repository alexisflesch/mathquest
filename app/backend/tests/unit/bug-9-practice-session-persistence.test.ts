...
const mockSocket = {
    emit: jest.fn((event: string, ...args: any[]) => {
        ...
        const handlers = (mockSocket as any).eventHandlers?.[event] || [];
        handlers.forEach((handler: Function) => handler(...args));
    }),
    on: jest.fn((event: string, handler: Function) => {
        // Store event handlers
        if (!(mockSocket as any).eventHandlers) {
            (mockSocket as any).eventHandlers = {};
        }
        if (!(mockSocket as any).eventHandlers[event]) {
            (mockSocket as any).eventHandlers[event] = [];
        }
        (mockSocket as any).eventHandlers[event].push(handler);
    }),
    disconnect: jest.fn(),
    connected: true
};
...
describe('Bug #9 - Practice Mode Session Persistence Vulnerability', () => {
    ...
    describe('Frontend Session Recovery (VULNERABLE)', () => {
        ...
        test('TC5: usePracticeSession hook lacks session recovery logic', () => {
            ...
            mockUseEffect(effectCallback);

            // Manually execute the effect (simulating React's behavior)
            effectCallback();
