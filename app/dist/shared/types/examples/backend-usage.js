...
function exampleSocketHandler(io, socket, payload) {
    ...
    io.to(`quiz:${question.uid}`).emit(index_1.TEACHER_EVENTS.GAME_CONTROL_STATE, {
        question,
        status: 'updated'
    });
}
...
function processQuestion(questionUid, data) {
    try {
        // Assert that questionUid is defined and of the right type
        (0, typeErrors_1.assertDefined)(questionUid, 'questionUid');
        (0, typeErrors_1.assertType)(questionUid, 'string', 'questionUid');
