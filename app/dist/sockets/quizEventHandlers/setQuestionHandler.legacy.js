...
const { patchQuizStateForBroadcast, initializeChrono, calculateRemainingTime, updateQuestionTimer, emitQuizTimerUpdate } = require('../quizUtils.legacy.js');
...
async function handleSetQuestion(io, socket, prisma, { quizId, questionUid, code: tournamentCode, teacherId }) {
    ...
    if (liveTournamentCode) {
        try {
            ...
            // Always ensure tournament status is 'en cours' when a teacher plays a question
            if (tournoi) {
                logger.info(`[SetQuestion] Ensuring statut 'en cours' for tournament ${liveTournamentCode}. linkedQuizId=${tournoi.linkedQuizId || 'none'}`);
                // Always update the status regardless of current state to ensure consistency
                try {
                    ...
                    logger.info(`[SetQuestion] CRITICAL: Updated Tournoi ${liveTournamentCode} status to "en cours" in database and memory`);
                }
                catch (statusUpdateErr) {
                    logger.error(`[SetQuestion] ERROR updating tournament status: ${statusUpdateErr}`);
                }
                // Always emit redirect_to_tournament when a teacher plays a question, regardless of tournament status
                ...
                // Broadcast to ALL lobby sockets in case the room membership isn't properly set
                logger.info(`[SetQuestion] Broadcasting global tournament_notification for quiz-linked tournament ${liveTournamentCode}`);
                io.emit("tournament_notification", {
                    type: "redirect",
                    code: liveTournamentCode,
                    message: "Tournament has started",
                    isQuizMode: true, // Explicitly mark as quiz mode to skip countdown
                    immediate: true // Flag for immediate redirect
                });
                // Send multiple redirect events with delays to ensure clients receive them
                const sendRedirectEvents = () => {
                    logger.info(`[SetQuestion] Sending follow-up redirect_to_tournament for tournament ${liveTournamentCode}`);
                    io.to(`lobby_${liveTournamentCode}`).emit("redirect_to_tournament", { code: liveTournamentCode });
                };
                // Send multiple redirect attempts with varying delays
                setTimeout(sendRedirectEvents, 500);
                setTimeout(sendRedirectEvents, 1500);
                setTimeout(sendRedirectEvents, 3000);
            }
            else {
                // Handle non-quiz-linked tournaments
                if (tournamentState[liveTournamentCode].statut !== 'en cours') {
                    logger.info(`[SetQuestion] Updating status to 'en cours' for non-quiz-linked tournament ${liveTournamentCode}`);
                    await prisma.tournoi.update({
                        where: { code: liveTournamentCode },
                        data: { statut: "en cours" }
                    });
