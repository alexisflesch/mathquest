...
const { patchQuizStateForBroadcast, calculateRemainingTime, emitQuizTimerUpdate, synchronizeTimerValues } = require('../quizUtils.legacy.js');
    ...
    if (!quizState_js_1.quizState[quizId] || quizState_js_1.quizState[quizId].profTeacherId !== teacherId) {
        ...
        prisma.quiz.findUnique({ where: { id: quizId }, select: { enseignant_id: true } }).then(async (quiz) => {
            ...
            if (code) {
                ...
                remaining = calculateRemainingTime(quizState_js_1.quizState[quizId].chrono, quizState_js_1.quizState[quizId].timerTimestamp);
            ...
            quizState_js_1.quizState[quizId].chrono.timeLeft = remaining; // Update quiz state remaining time
            quizState_js_1.quizState[quizId].chrono.running = false;
            // Mark this quiz as explicitly handled by pauseHandler to avoid duplicate processing
            quizState_js_1.quizState[quizId].pauseHandled = Date.now();
