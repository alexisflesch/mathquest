...
async function handleJoinQuiz(io, socket, prisma, { quizId, role, teacherId }) {
    ...
    if (quizState_1.quizState[quizId]?.questionTimers && Object.keys(quizState_1.quizState[quizId]?.questionTimers || {}).length > 0) {
        const orderedQuestions = quizState_1.quizState[quizId].questions;
        orderedQuestions.forEach(q => {
            const timers = quizState_1.quizState[quizId]?.questionTimers || {};
            if (q && q.uid &&
                timers[q.uid] &&
