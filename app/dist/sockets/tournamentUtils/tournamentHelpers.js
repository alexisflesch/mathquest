...
const tournamentState_1 = require("./tournamentState"); // Changed to ES6 import, assuming tournamentState.ts is the source or has a bridge
const sendTournamentQuestion_1 = require("./sendTournamentQuestion"); // Changed to ES6 import, assuming .ts or bridge
const scoreUtils_1 = require("./scoreUtils"); // Changed to ES6 import, assuming .ts or bridge
const db_1 = __importDefault(require("../../db")); // Changed to ES6 import
/**
...
function sendQuestionWithState(io, code, questionIndex, questionUid = undefined, targetRoom = null, isDiffered = false) {
    ...
    try {
        (0, sendTournamentQuestion_1.sendTournamentQuestion)(target, {
            ...
            questionIndex,
            questionId
            // targetRoom is no longer part of the payload for sendTournamentQuestion
        });
    }
    catch (err) {
    ...
    const answeredCount = state.participants?.filter((p) => p.answers?.some((a) => a.questionUid === questionId)).length || 0;
