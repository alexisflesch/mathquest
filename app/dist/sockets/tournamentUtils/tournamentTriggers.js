...
const tournamentHelpers_1 = require("./tournamentHelpers");
// Import scoring functions from scoreUtils
...
function triggerTournamentQuestion(io, code, index, linkedQuizId = null, initialTime = null, targetQuestionUid = null) {
    ...
    try {
        (0, tournamentHelpers_1.sendQuestionWithState)(io, code, targetIndex, questionUid);
    }
    catch (error) {
        logger.error(`[TriggerQuestion] Error in sendQuestionWithState: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    if (linkedQuizId && initialTime !== null) {
        ...
        const teacherSocketId = quizState_1.quizState[linkedQuizId]?.profSocketId;
        if (teacherSocketId) {
...
function triggerTournamentTimerSet(io, code, timeLeft, forceQuestionState = null) {
    ...
    if (timeLeft === 0 && questionUid) {
        (0, tournamentHelpers_1.handleTimerExpiration)(io, code, questionUid);
    }
}
/**
...
function triggerTournamentAnswer(io, code, participantId, answer, questionUid) {
    ...
    try {
        const { totalScore, baseScore, timePenalty } = (0, scoreUtils_1.calculateScore)(question, answerObject, timerState?.lastUpdateTime || now - answerTimeMs, state.questions.length);
