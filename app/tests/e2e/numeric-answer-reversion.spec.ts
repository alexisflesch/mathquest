/**
 * E2E Test: Numeric Answer Reversion
 *
 * This test verifies that when a student submits a numeric answer that gets accepted,
 * then tries to change it too late and the submission is rejected, the UI reverts
 * to showing the previously accepted answer.
 *
...
async function createQuizViaAPI(page: Page, teacherData: any): Promise<QuizData> {
    ...
    try {
        ...
        // Step 1: Get some question UIDs - look for CE1 maths questions which should include numeric ones
...
test.describe('Numeric Answer Reversion', () => {
    test('should revert to previously accepted numeric answer when late submission is rejected', async ({ browser }) => {
        ...
        try {
            ...
            // Generate test data
            const teacherData = dataHelper.generateTestData('numeric_teacher');
            ...
            const studentData = dataHelper.generateTestData('numeric_student');
