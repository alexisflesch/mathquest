...
test.describe('Tournament Mode', () => {
    test('tournament creation and participation', async ({ browser }) => {
        ...
        try {
            ...
            // Teacher navigates to lobby (validates tournament creation works)
            await teacherPage.goto(`${TEST_CONFIG.baseUrl}/lobby/${tournamentData.accessCode}`);
            await teacherPage.waitForLoadState('networkidle');

            // Verify teacher is on the lobby page
            await expect(teacherPage).toHaveURL(new RegExp(`/lobby/${tournamentData.accessCode}`));

            log('✅ Tournament creation and navigation works (core functionality validated)');
