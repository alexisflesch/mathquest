...
### 🔍 Advanced Static Analysis and Code Cleanup

To maintain a clean and consistent codebase, several static analysis tools are used in addition to standard linters. For example, `jscpd` detects duplicate or near-duplicate code blocks, while `eslint-plugin-sonarjs` flags suspicious patterns or unnecessary complexity. Tools like `depcheck` and `madge` help identify unused dependencies and circular imports respectively. These tools are essential for preventing technical debt accumulation.

---

### 🧪 Custom Quality Control Scripts

Custom Python scripts are being developed to catch semantic redundancies in the code:
- A **TypeScript interface similarity checker**, using `ts-morph`, detects locally defined interfaces that could be unified or generalized.
- A **navigation analyzer**, powered by `Puppeteer`, dynamically explores the app’s navigation graph to detect defined routes that are unreachable via normal user flows (orphaned pages).

These tools are designed to enforce standards and highlight patterns that AI agents and automated PRs often overlook.
