...
import createLogger from '@logger'; // Use path alias
...
function triggerTournamentQuestion(
    ...
    try {
        sendQuestionWithState(io, code, targetIndex, questionUid);
    } catch (error) {
...
function triggerTournamentAnswer(
    ...
    try {
        const { totalScore, baseScore, timePenalty } = calculateScore(
            question,
            answerObject,
            timerState?.lastUpdateTime || now - answerTimeMs,
